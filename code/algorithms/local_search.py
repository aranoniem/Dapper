...
class Local_search():
    """
    create a semi random railnetwork but if the qualityscore isn't improved,
    do not output the new score
    ...
    def solve(self, max_trajectory: int, timeframe: int) -> float:
        ...
        for i in range(1000):
            i += 1
            temp_station = self.random_station(self.data)
            temp_trajectory = [temp_station]
            temp_visited_stations = {temp_station}
            temp_duration = 0
            ...
            while True:
                ...
                #stop making connections when timeframe is reached
                temp_duration += self.data[station].get_distance(random_neighbour)
                if temp_duration > timeframe:
                    break

                #add station to trajectory
                temp_trajectory.append(random_neighbour)

                #remember stations that are visited in trajectory
                temp_visited_stations.add(random_neighbour)
                station = random_neighbour

            
            for i in range(max_trajectory):
                temp_railnetwork[i] = temp_trajectory
                temp_quality_score = Score(self.level, temp_railnetwork, timeframe)
